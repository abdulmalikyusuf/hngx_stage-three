...
function SignIn() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // if(err.error[0].code === "form_identifier_not_found"){
    ...
    <div className="flex flex-row w-full">
      ...
      <div className='hidden lg:flex flex-col justify-between bg-[#ffe85c] lg:p-8 xl:p-12 lg:max-w-sm xl:max-w-lg'>
        <div className="flex items-center justify-start space-x-3">
          ...
          <a href="#" className="text-xl font-medium">TheLouvre</a>
        </div>
        <div className='space-y-5'>
          <h1 className="font-extrabold lg:text-3xl xl:text-5xl xl:leading-snug">Enter your account and discover new
            experiences</h1>
          <p className="text-lg">You do not have an account?</p>
          <button
            className="flex-none inline-block px-4 py-3 font-medium text-white bg-black border-2 border-black rounded-lg">Create
            account here</button>
        </div>
      ...
      <div className="relative flex flex-col items-center justify-center flex-1 px-10">
        ...
        {/* <!-- Login box --> */}
        <form onSubmit={handleSubmit} className="flex flex-col justify-center flex-1 max-w-md space-y-5">
          <div className="flex flex-col space-y-2 text-center">
            <h2 className="text-3xl font-bold md:text-4xl">Sign in to account</h2>
            <p className="text-md md:text-xl">Log in to experience stunning images! Lorem ipsum dolor sit amet consectetur adipisicing elit.</p>
          </div>
          <div className="flex flex-col max-w-md space-y-5">
            <input type="email" name="emailAddress" autoComplete="emailAddress" value={emailAddress} onChange={e=>setEmailAddress(e.target.value)} placeholder="Email address"
              className="flex px-3 py-2 font-medium border-2 border-black rounded-lg md:px-4 md:py-3 placeholder:font-normal" />
            <input type="password" name="password" autoComplete="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="********"
              className="flex px-3 py-2 font-medium border-2 border-black rounded-lg md:px-4 md:py-3 placeholder:font-normal" />
            <button type="submit" className="flex items-center justify-center flex-none px-3 py-2 font-medium text-white bg-black border-2 border-black rounded-lg md:px-4 md:py-3">Login
              with email</button>
              {errors && errors[0]?.code === "form_identifier_not_found" && <p className="font-medium text-red-400">Email or password is incorrect</p>}
        ...
        <div className="flex flex-col justify-center m-auto mb-16 text-lg text-center dark:text-slate-200 ">
          <p className="mb-1 font-bold">Built by <a href="#" className="underline dark:text-white">Frank Esteban</a></p>
          <p>Contact me on the different platforms and social networks</p>
          <div className="flex flex-wrap items-center justify-center mt-4 space-x-2"><a
              href="https://www.linkedin.com/in/frankuxui/"
              className="flex items-center justify-center flex-none w-12 h-12 transition-all rounded-full hover:bg-slate-200 dark:hover:bg-slate-700"><svg
